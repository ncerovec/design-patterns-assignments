...
public class AlgorithmMaxLevel implements Algorithm
{
    ...
    public List<DiveCombination> makeTroika(List<DiveCombination> diveCmbs, List<Diver> groupDivers, int depthLimit)
    {
        ...
        if(listSize%2 > 0)
        {
            ...
            {
                DiveTroika troika = new DiveTroika();
                ...
                for(int j=i-1; j >= 0; j--)
                {
                    ...
                    {                        
                        for(int k=j-1; k >= 0; k--)
                        {
                            ...
                            {
                                troika.setDiver1(anchorDiver);
                                troika.setDiver2(secondDiver);
                                troika.setDiver3(thirdDiver);
        ...
        for(int i=listSize-1; i >= 0; i--)
        {
            ...
            if(listSize != 3)   //can't pair 3 divers - combine last divers to Troika
            {
                for(int j=i-1; j >= 0; j--)
                {
                    ...
                    {
                        pair.setDiver1(anchorDiver);
                        pair.setDiver2(pairDiver);
