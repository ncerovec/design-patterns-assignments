...
import ncerovec_zadaca_4.notifications.NotificationSubject;
...
import ncerovec_zadaca_4.table.TableDives;
import ncerovec_zadaca_4.table.TableDivingStats;

/**
 * SINGLETON - Class
 * OBSERVER - ConcreteSubject
 * LAZY ACQUISITION - Instance
 * EAGER ACQUISITION - Instance
 * @author nino
 */
public class Club implements NotificationSubject
{
    ...
    private List<EquipmentCategory> equipmentStock = new ArrayList();   //eager loading instantiation (example)
    ...
    public static Club getInstance() //lazy loading instantiation (example)
    {
        if(INSTANCE == null)
        {
    ...
    public List<EquipmentCategory> getEquipmentStock() { return equipmentStock; }
    public void setEquipmentStock(List<EquipmentCategory> equipment) { this.equipmentStock = equipment; }
    ...
    public Date getMostRecentDive(Diver diver)
    {
        Date recentDiveDate = null;
        
        List<Dive> diverDives = this.getDiverDives(diver);
        for(Dive dive : diverDives)
        {
            if(recentDiveDate == null || dive.getDateTime().getTime() > recentDiveDate.getTime())
            {
                recentDiveDate = dive.getDateTime();
            }
        }
        
        return recentDiveDate;
    }
    
    public EquipmentPart getEquipment(String equipmentId)
    {
        for(EquipmentCategory eqCtg : this.getEquipmentStock())
        {
            for(EquipmentPart eqPart : eqCtg.getEquipment())
            {
                if(eqPart.getId().equals(equipmentId)) return eqPart;
            }
        }
